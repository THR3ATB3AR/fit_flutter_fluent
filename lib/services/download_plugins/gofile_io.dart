...
class GofileIo {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 'ok' && data['data']['token'] != null) {
          ...
          throw Exception(
            'Gofile API did not return a valid token. Response: ${response.body}',
          );
        }
      } else {
        throw Exception(
          'Failed to get Gofile guest token, status: ${response.statusCode}, body: ${response.body}',
        );
      }
    } catch (e) {
  ...
  Future<List<DownloadInfo>> getDownloadInfo(
    String repackTitle,
    String gofileUrl,
  ) async {
    ...
    if (folderId == null) {
      throw ArgumentError(
        'Invalid Gofile URL format. Could not find a folder ID.',
      );
    }
    ...
    final contentUrl = Uri.parse(
      'https://api.gofile.io/contents/$folderId?wt=4&cache=true',
    );
    ...
    try {
      ...
      if (response.statusCode != 200) {
        String errorMsg =
            'Failed to get Gofile content, status code: ${response.statusCode}';
        try {
          ...
          if (status == 'error-notPremium') {
            ...
            debugPrint(
              '[Gofile] Server is overloaded and requires a premium account for this file.',
            );
            throw Exception(
              'Gofile server is busy. This file currently requires a Premium account to access. Please try again later.',
            );
          } else if (status != null) {
      ...
      final data = jsonDecode(response.body);
      if (data['status'] != 'ok') {
        if (data['status'] == 'error-passwordRequired' ||
            data['status'] == 'error-passwordWrong') {
          throw Exception(
            'This Gofile folder is password protected, which is not supported.',
          );
        }
      ...
      if (contents == null)
        throw Exception('Gofile response is missing "contents" data.');

      final Map<String, dynamic>? children = data['data']?['children'];
      if (children == null)
        throw Exception('Gofile response is missing "children" data.');

      final List<DownloadInfo> downloadInfos = [];
      final rootFolderId = data['data']?['id'];
      if (rootFolderId == null)
        throw Exception(
          'Could not determine the root folder ID from the response.',
        );

      final rootFolderData = contents[rootFolderId];
      if (rootFolderData == null ||
          rootFolderData['type'] != 'folder' ||
          rootFolderData['children'] == null) {
        throw Exception(
          'Root content is not a valid folder or has no children.',
        );
      }

      final List<String> childrenIds = List<String>.from(
        rootFolderData['children'],
      );
      ...
      if (downloadInfos.isEmpty)
        throw Exception('No valid files found in the Gofile folder.');

      debugPrint(
        '[Gofile] Successfully parsed ${downloadInfos.length} file(s).',
      );
