...
class GogDownloadLinksDialog extends StatefulWidget {
  final String gogGameTitle;
  final String mirrorUrlsString; 
  final String downloadPath;
  final HostService hostService;

  const GogDownloadLinksDialog({
    super.key,
    required this.gogGameTitle,
  ...
  @override
  State<GogDownloadLinksDialog> createState() => _GogDownloadLinksDialogState();
}

class _GogDownloadLinksDialogState extends State<GogDownloadLinksDialog> {
  ...
  Future<void> _fetchAndProcessLinks() async {
    ...
    for (final url in urls) {
      ...
      try {
        final result = await widget.hostService.getDownloadPlugin(widget.gogGameTitle, url);
        if (result is DownloadInfo) {
          ...
          collectedInfos.add(DownloadInfo(
              repackTitle: widget.gogGameTitle,
              downloadLink: url,
  ...
  if (itemsToDownload.isNotEmpty) {
    ...
    for (var item in itemsToDownload) {
      await ddManager.addDdLink(item, widget.downloadPath, widget.gogGameTitle);
    }
  ...
  Widget build(BuildContext context) {
    ...
    return ContentDialog(
      title: Text(AppLocalizations.of(context)!.downloadFilesGame(widget.gogGameTitle)),
